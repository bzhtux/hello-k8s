...
def get_keys_from_config(config_file):
    ''' Get all configuration keys from configuration file in `instance/` directory.
        ...
        >>> from hka.utils import get_keys_from_config
        >>> from hka import app
        ...
        'TOKEN_SRV', 'USERS_SRV', 'HKA_hka_ENDPOINT', 'HKA_hka_PORT', 'HKA_SECRET_KEY',
        'HKA_TOP_LEVEL_DIR', 'HKA_EMAIL_VERIFY', 'HKA_DEBUG', 'HKA_ADMIN_PASS',
        'HKA_TOKEN_VALID_DURATION', 'HKA_DB_CONNECTOR', 'HKA_DB_NAME', 'SQLALCHEMY_DATABASE_URI',
...
def set_config_keys(config_dict):
    ''' Set new configuration app.config[$KEYS].
        ...
        >>> from hka.utils import set_config_keys
...
def use_env_vars():
    ...
    if len(conf) > 0:
        if set_config_keys(conf) and app.config['HKA_DEBUG']:
